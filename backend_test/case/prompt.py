...
{{
    ...
    "study_area": {{
        ...
        "region": "省/州/地区",
        "site_name": "具体站点名称",
        "coordinates": {{"lat": "纬度", "lon": "经度"}}, # 只有文中明确提到才进行提取
    }},
    "experiment_time": {{
        "start_date": "起始日期",
        "end_date": "结束日期",
...
###########################################################
############ ADMINISTRATION_ALIGNMENT_PROMPT ############ 
###########################################################
...
# 任务
根据用户提供的地理位置描述信息，将其对齐到标准的中国省、市、县三级行政区划名称。

# 标准行政区划名称参考列表
## 省级（共 {province_count} 个）
'河南省', '浙江省', '海南省','台湾省', '甘肃省', '湖北省',  '天津市',  '内蒙古自治区',    '广西壮族自治区',  '江苏省',    '山东省',  '北京市',  '西藏自治区',  '青海省',    '重庆市',  '澳门特别行政区',  '吉林省',    '福建省',  '广东省',  '上海市',  '江西省',  '香港特别行政区',  '黑龙江省',  '山西省',    '云南省',  '四川省',  '安徽省',  '宁夏回族自治区',  '辽宁省',    '湖南省',  '陕西省',  '河北省',  '新疆维吾尔自治区',    '贵州省' 

## 市级（共 {city_count} 个，仅列出可能相关的部分）
{city_list}

## 县级（共 {county_count} 个，仅列出可能相关的部分）
{county_list}

# 对齐规则
1. **优先使用地名文本信息**进行匹配，经纬度作为辅助验证
2. 必须从上述参考列表中选择，不得自行编造行政区名
3. 处理以下常见情况：
   - 英文地名翻译为中文（如 "Henan" → "河南省"）
   - 简称补全（如 "河南" → "河南省"，"商丘" → "商丘市"）
   - 旧称/别名转换（如 "奉天" → "沈阳市"）
   - 模糊描述推断（如 "华北平原某农业试验站" 结合经纬度判断）
   - 拼写错误容错（如 "Shagnqiu" → "商丘市"）
4. 如果该地点不在中国境内，所有字段填 null
5. 如果信息不足以确定某一级别，该级别填 null
6. 经纬度可辅助判断：根据坐标大致范围确认省市归属

# 输出格式
严格输出以下 JSON，不要输出任何其他内容：
{{"province": "省级名称或null", "city": "市级名称或null", "county": "县级名称或null", "confidence": "high/medium/low", "reasoning": "简要推理过程"}}

# 输入信息
- country（国家）: {country}
- region（省/州/地区）: {region}
- site_name（站点名称）: {site_name}
- latitude（纬度）: {latitude}
- longitude（经度）: {longitude}
- land_cover（土地覆盖类型）: {land_cover}
